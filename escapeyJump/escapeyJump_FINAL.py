...
class Player(pg.sprite.Sprite): #Creates the player/user
	...
	def __init__(self, game): #constructs player features and image
		...
		self.rect.x = (WIDTH/5) #centers avatar
	...
	def jump(self): #jump movement only when not in air
		...
		diff = HEIGHT - self.rect.top		
		self.rect.x += 1 #trying to unsnap sprite
		hits = pg.sprite.spritecollide(self, self.game.platforms, False)
		if hits: #dont move unless we are on solid
			self.game.leapsound.play()			
			self.rect.x -= 1			
			self.vy -= jumpvelocity
			for form in self.game.platforms:			
				form.rect.top += (abs(diff)-10)
				if form.rect.top > HEIGHT+100: #leave some in case we fall
					form.kill() #get rid of platforms downbelow
					self.game.points += 10 #increase our point total
					print 'DELETED'		
		else: #if we arent, move
			self.dy = 0

	def stop(self):
		'''FIXED _DEBUG: Not being called correctly in events()'''
		#if we hit anything don't move
		hits = pg.sprite.spritecollide(self, self.game.platforms, False)
		if hits: #stop moving when KEYUP on platform	
			self.vx = 0
			self.vy = 0
...
class Game: #This is the jumper game
	...
	def new(self):
		...
		self.leapsound = pg.mixer.Sound(path.join(self.dir, 'leap.ogg'))
		self.run()
	...
	def update(self): #updating while running
		...
		if hits:
			...
			'''Remember: to set starting point on bottom platform '''
			self.player.rect.y = hits[0].rect.top - 20
			self.vy = 0 #stop motion

		#Scroll further up once reach checkpoint (VERTICAL)
		if self.player.rect.y <= (HEIGHT/8): 
			print 'PAST CHECKPOINT'
			#if self.player.rect.top <= HEIGHT: #when reach 1/6 of height
			''' FIXED  _DEBUG: Not scrolling '''			
			#Move at vel of player in opp direction'''
			self.player.rect.y += abs(self.player.vy)			
			diff = HEIGHT - self.player.rect.top
			#self.player.rect.y = HEIGHT - 100 #Debug_REMOVED (2)				
			
		#Make new platforms
		while len(self.platforms) < 8: #num of platforms in set
		...
		#GAME OVER
		if self.player.rect.top > HEIGHT: #player falls off
	...
	def shiftworld(self, shiftx, shifty): #shift the screen
		self.worldup += abs(self.player.vy)
