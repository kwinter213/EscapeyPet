...
'''
***** UPDATES *******
FIXED: ypos first setting; jump; stop; 
**Need to look at calling the jump 'leap' sound
WORKING ON: scroll concept (a bit rough); leap.sound
'''
...
class Game: #This is the jumper game
	...
	def update(self): #updating while running
		...
		if self.player.rect.y <= (HEIGHT/3): 
			...
			#self.player.rect.y += abs(self.player.vy) #ADDING THIS (1)			
			diff = HEIGHT - self.player.rect.top
			#self.player.rect.y = HEIGHT - 100 #Debug_REMOVED (2)				
			for form in self.platforms:
				'''self.shiftworld(-diff) #platforms.rect.y += shiftup'''
				form.rect.y += abs(diff) #decrease height by vel
				print str(abs(diff)) + 'DIFF+++'
			...
			#Make new platforms
			while len(self.platforms) < 6: #num of platforms in set
